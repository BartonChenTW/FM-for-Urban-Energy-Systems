...
{: .note }
In this book's domain, simulators are an alternative to both: a simulator can generate unlimited *labelled* examples (configuration in, trajectory out) without any human labelling step at all. This is why [§4.10.1](../chapter-4-directions/4-10-building-it.html#4101-data-generation-and-sampling-design) treats "the training distribution is a design decision" as the central data question here, rather than "how do we get labels" — see also the discussion of synthetic data limits in [§3.2](../chapter-3-sim-opt/3-2-building-simulation-data.html).
