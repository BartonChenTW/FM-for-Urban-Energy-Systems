...
5. **You may be building a very expensive interpolator.** If Phase 1 surrogates ([§5.1](5-1-roadmap.html)) do not beat a well-tuned reduced-order model on held-out topologies, the FM premise fails. This is why the [minimum viable v0](5-5-token-schema.html#555-minimum-viable-v0) builds this kill criterion in explicitly, and why the amortisation argument in [§3.5](../part-3-sim-opt/3-5-design-sizing-optimisation.html) only pays off if transfer actually holds.
...
{: .note }
These risks and open questions are deliberately kept separate from [Part 6's open gaps](../part-6-outlook/6-1-open-gaps.html), which are framed as research contributions available to the wider field. The items here are specific to this one case study's engineering choices, not general statements about what UES-FM research needs.

---
[← Previous: 5.7 Module and Task Decomposition](5-7-module-decomposition.html) · [Back to Part 5](index.html) · [Next: Part 6 — Outlook →](../part-6-outlook/index.html)
