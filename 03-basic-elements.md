...
D1 and D2 (see [FM Fundamentals](02-fm-fundamentals.html)) are usually treated as implementation detail. They are not. The choice of basic element determines what transfers, and no amount of architecture or compute compensates for a bad one.
...
## 6.7 Basic elements for buildings
...
**(d) The time interval / patch.** A fixed-length slice of hourly output. Unambiguous, stable and scale-independent — this is what time-series foundation models use (see [The FM Landscape](04-fm-landscape.html)). But it describes the *output*, not the object. A slice of a demand profile carries no information about the building that produced it, so building characteristics must be attached from outside.

**(e) The cell.** A single entry in a table — where one row meets one column. Scores better against the four requirements than (a)–(d) and fails on time-resolved output. Treated in full in [The FM Landscape §7.4](04-fm-landscape.html#74-tabular-foundation-models--the-cell-as-a-basic-element).
...
## 6.8 Representation strategies and testable predictions
...
**R3 — Building as a network of connected parts.** Zones or components as nodes, thermal and hydraulic couplings as edges — closest to how building physics already models. Highest transfer potential, and pays the zoning ambiguity cost of §6.7(b) in full. **The more principled option, currently blocked by an unsolved problem belonging to the building simulation community rather than the ML one.**

**R4 — Building as an element of a stock.** Individual buildings as the basic element within a portfolio. Suits stock-level questions; gives up within-building resolution.
...
---
[← Previous: FM Fundamentals](02-fm-fundamentals.html) · [Next: The FM Landscape →](04-fm-landscape.html)
