...
- **Scaling laws now hold for time series.** Toto 2.0 is the first time-series model where classic scaling behaviour (more data and parameters → predictably better performance) has been demonstrated.[^cohen2025toto] This matters because scaling laws are what justified the FM bet in language in the first place — see [§2.6](../chapter-2-fm-foundations/2-6-scaling-laws.html).
