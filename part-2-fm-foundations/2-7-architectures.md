...
ML-basics content for readers arriving from the energy-systems side. Covers what each architecture family actually does, aimed at the level needed to follow [Part 4](../part-4-directions/index.html) and [Part 5](../part-5-case-study/index.html) — not a complete technical treatment.
{: .fs-6 .fw-300 }
...
Two concrete families: the **Fourier Neural Operator** learns mappings between function spaces using spectral (Fourier-domain) parameterisations, targeting families of parametric partial differential equations rather than one fixed instance; **DeepONet** learns solution operators directly, splitting the network into a branch that encodes the input function and a trunk that encodes the query location. See [§4.9.2](../part-4-directions/4-9-2-methods-tier2.html) for when to choose a neural operator over a GNN in this domain.
