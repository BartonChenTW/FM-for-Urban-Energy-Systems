...
Before building anything bespoke, the cheapest and most immediately useful thing a UES practitioner can do is evaluate an already-pretrained, general-purpose time-series foundation model **zero-shot** on their own forecasting problem — no training, no fine-tuning, just point the model at the series and read off a forecast. The current generation of these models (Chronos-2, TimesFM 2.5, Moirai 2.0, TabPFN-TS — see [§2.4.1](../part-2-fm-foundations/2-4-1-time-series-fms.html)) is production-grade and free or cheap to run.

This is directly applicable to **load forecasting** — predicting building or district electricity, heat, or cooling demand a few hours to days ahead (T3 in [§3.1](../part-3-sim-opt/3-1-taxonomy-of-tasks.html)) — which is already flagged as a task with mature, off-the-shelf solutions in [§4.7](4-7-reading-the-screen.html).
...
- Notes on which covariates (weather, calendar, building metadata) each model can actually ingest zero-shot, referencing the covariate-handling differences in [§2.4.1](../part-2-fm-foundations/2-4-1-time-series-fms.html).
...
---
[← Back to Part 4](index.html) · [Next: 4.2 FMs for Whole Building Stocks →](4-2-fms-for-building-stocks.html)
